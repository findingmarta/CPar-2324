...
#define NUM_BLOCKS static_cast<int>(std::ceil(static_cast<float>(N) / static_cast<float>(NUM_THREADS_PER_BLOCK)))
...
//  Vectors!
//
const int MAXPART = N+1;
double* POT_RESULTS = (double *) malloc(NUM_BLOCKS*sizeof(double));
//  Position
double* r = (double *) malloc(3*MAXPART*sizeof(double));
//  Velocity
double v[3][MAXPART];
//  Acceleration
        ...
        for (j=0; j<n; j++) {
            for (k=0; k<n; k++) {
                if (p<N) {
                    ...
                    r[p] = (i + 0.5)*pos;
                    r[MAXPART + p] = (j + 0.5)*pos;
                    r[2*MAXPART + p] = (k + 0.5)*pos;
                }
                p++;
            }
        }
...
__global__ void computeAccelerations(double* r, double* a, double* Pot) {
    int bid = blockIdx.x;
    int tid = threadIdx.x;
    int i = bid* blockDim.x + tid, j;
   ...
   // position of i relative to j
    ...
    //double sigma2 = sigma * sigma;
        ...
        __shared__ double aux[NUM_THREADS_PER_BLOCK];
        aux[tid]=0;
        for (j = i+1; j < N; j++) {
            ...
            rij[0] = r[i] - r[j];
            rij[1] = r[MAXPART + i] - r[MAXPART + j];
            rij[2] = r[2*MAXPART + i] - r[2*MAXPART + j];    
            ...
            term1 = term2 * term2;
           // atomicAddDouble(Pot, term1 - term2); 
            aux[tid] +=  term1 - term2;
            ...
            a0i += rij[0] * f;
            atomicAddDouble(&a[j], -rij[0] * f);
            a1i += rij[1] * f;
            atomicAddDouble(&a[MAXPART + j], -rij[1] * f);
            a2i += rij[2] * f;
            atomicAddDouble(&a[2*MAXPART + j], -rij[2] * f);
        }
        atomicAddDouble(&a[i], a0i);
        atomicAddDouble(&a[MAXPART + i], a1i);
        atomicAddDouble(&a[2*MAXPART + i], a2i);
...
void computeAccelerationsAndPotential() {
    ...
    cudaMemset(d_a, 0, sizeof(double) * MAXPART * 3);
    cudaMemset(d_Pot, 0, sizeof(double)*NUM_BLOCKS);
    checkCUDAError("set memory");

    computeAccelerations<<<NUM_BLOCKS,NUM_THREADS_PER_BLOCK>>>(d_r, d_a, d_Pot);
    checkCUDAError("memcopy1");

    cudaMemcpy(a, d_a, sizeof(double) * MAXPART * 3, cudaMemcpyDeviceToHost);
    checkCUDAError("memcopy2");

    cudaMemcpy(POT_RESULTS, d_Pot, sizeof(double)*NUM_BLOCKS, cudaMemcpyDeviceToHost);
    checkCUDAError("memcopy3");
...
double VelocityVerlet(double dt, int iter, FILE *fp) {
    ...
    for (j=0; j<3; j++) {
        for (i=0; i<N; i++) {
            r[j*MAXPART + i] += dt * (v[j][i] + dt_05*a[j*MAXPART + i]);
            v[j][i] += dt_05*a[j*MAXPART + i];        
        }
    ...
    for (j=0; j<3; j++) {
        for (i=0; i<N; i++) {
            if (r[j*MAXPART + i]<0.) {
                ...
                psum += dt_2m*fabs(v[j][i]);  // contribution to pressure from "left" walls
            }
            if (r[j*MAXPART + i]>=L) {
                v[j][i]*=-1.;  //- elastic walls
                psum += dt_2m*fabs(v[j][i]);  // contribution to pressure from "right" walls
            }
        }
    }
